...
sintagma_nominal([N], Q):- nombre(N,_,Q).
sintagma_nominal([],_).
...
descomponer([N|L],X1,Y1,[],X2,Y2,Z2):- verbo(N,_), descomponer(L,X1,[N|Y1],[],X2,Y2,Z2).
descomponer([N|L],X1,Y1,[],X2,Y2,Z2):- (determinante(N,_,_) ; nombre(N,_,_)), descomponer(L,X1,Y1,[N],X2,Y2,Z2).
descomponer([N|L],X1,Y1,Z1,X2,Y2,Z2):- (determinante(N,_,_) ; nombre(N,_,_)), descomponer(L,X1,Y1,[N|Z1],X2,Y2,Z2).
...
oracion_bnf(X,Y,Z):- sintagma_nominal(X,Q), sintagma_verbal([Y,Z],Q).
