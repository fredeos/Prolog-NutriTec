...
% >> Sintagmas verbales
sintagma_verbal([V], Num):- verbo(V, Num).
sintagma_verbal([V,SN], Num):- verbo(V, Num), sintagma_nominal(SN, Num).

% >> Sintagmas nominales
sintagma_nominal([D,N], Num):- determinante(D,G,Num), nombre(N,G,Num). %REGLA:
...
oracion(X,Y,Z):- sintagma_nominal(X, Num), sintagma_verbal([Y,Z], Num).
